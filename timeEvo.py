...
def make_fig(pos_pa, i):
    ...
    ax.scatter(pos_pa[:,0], pos_pa[:,1], s=4, c='black', alpha=0.3)
    ax.set_xlim((-80, 80))
    ax.set_ylim((-80, 80))
    ax.set_xlabel('x [kpc]')
    ax.set_ylabel('z [kpc]')
    fig.savefig('/mnt/home/npanithanpaisal/darkpy/figs/xy_snap{}.png'.format(i), dpi=300)
    print('xy_snap{}.png has been created'.format(i))
    plt.close(fig)
